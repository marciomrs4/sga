...
class Alteracao extends Dados
{
	...
	public function alterarProjeto()
	{
		try
		{
			...
			if(($this->dados['stp_codigo'] != 2) and ($quantidade != 0)){
				throw new Exception('Existe atividade em andamento: Este projeto n�o pode ser alterado');
			}
	...
	public function alterarSolicitacao($file)
	{
		try
		{
			...
			ValidarCampos::validarQtdCaracter($this->dados['sol_descricao_solicitacao'],10,'Descri��o do '.$_SESSION['config']['problema']);
	...
	public function alterarAtendimentoSolicitacao()
	{
		...
		try
		{
			...
			ValidarCampos::campoVazio($this->dados['sol_descricao_solucao'],'Descri��o da Solu��o');
			ValidarCampos::validarQtdCaracter($this->dados['sol_descricao_solucao'],10,'Descri��o da Solu��o');
	...
	public function alterarAtenderChamado($sol_codigo)
	{
		try
		{
			...
			try
			{
				...
				if($atendente)
				{
					throw new Exception('J� existe um atendente para a ocorr�ncia','300');
				}else
				{
	...
	public function alterarAtividade()
	{
		try
		{
			...
			try
			{
				...
				if($status != 2)
				{
					throw new Exception('N�o � poss�vel alterar essa atividade: Este projeto n�o esta em andamento');
				}else
				{
	...
	public function alterarSolicitacaoMelhoria()
	{
		...
		if($tbMelhoria->getUsuarioSolicitante($this->dados['som_codigo']) != $_SESSION['usu_codigo']){
			throw new Exception('Impossivel alterar! Voc� n�o � o criador da solicita��o');
		}
